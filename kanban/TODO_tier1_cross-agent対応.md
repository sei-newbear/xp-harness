...
**痛みが顕在化 (2026-07-17)**: 依頼者が本業で Codex と Cursor を実際に使っており、そこでも xp-harness の規律を効かせたい。philosophy 4 象限で「痛みが明確化していない」に置いていた保留を解く根拠が揃い、backlog → tier1 に昇格。**まず効かせたい先は Codex と Cursor の 2 つに具体化** (Copilot / Gemini は当面スコープ外)。両者は機構が異なる (Codex は CLI / クラウド寄り、Cursor は IDE の rules 機構) ので、着手時の要件定義で対象と方式を締める。
...
1. 痛みは顕在化済み (本業で Codex / Cursor 利用、上記「状況」参照)。まず Codex と Cursor に絞る
2. Codex と Cursor の skill / agent (rules) 機構を調査
