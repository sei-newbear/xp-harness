...
description: 別セッションのエージェントへ引き継ぐ成果物 (要件定義・基本設計・引き継ぎ書・PRD・README 等) を会話文脈なしで点検し、文脈ゼロの引き継ぎ先が下流タスクに着手できるか (自己完結しているか) を Yes/No と穴の具体指摘で返す。成果物を別セッションへ引き継ぐ前、または自己完結だと宣言する前に呼ぶ。呼び出し側が渡すのは成果物のパスと下流タスクだけ (会話の背景・要約は渡さない)。要件・設計の妥当性は pre-implementation-reviewer、コードの Done 達成は done-verifier の責務。
