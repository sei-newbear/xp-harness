---
name: e2e-reviewer
description: E2E spec を書き終わった直後、または依頼者が手動で呼んだときに、新規追加・変更された E2E spec をペアプロ相手としてレビューする。プロジェクトの E2E の流儀に照らした点検と、要件定義の Done と spec の双方向照合を行う。呼び出し側は対象タイトルの要件定義（あれば基本設計も）のパスをプロンプトで渡すこと。コードは code-reviewer、要件定義 / 基本設計そのものは pre-implementation-reviewer の責務。
