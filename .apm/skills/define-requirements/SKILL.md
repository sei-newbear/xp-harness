...
## まず「深掘る / そのまま進める」を見立てて確認する ★最初に通す

発火したら、いきなり Why 掘りや下の 4 セクションの要件定義に入らない。まず依頼を軽く読み、これは一度 Why を深掘る価値があるか、それとも掘らずそのまま進めてよいかを見立てる。

- **深掘る**（Why を掘って、指定された手段より良いやり方がないか探る）に寄せる目安: 依頼が特定の手段・解決策を指していて、その裏の Why 次第では別のやり方の方が良いかもしれないとき。「○○を表示して」「○○を追加して」のような解決策の指定は、それ自体が深掘る理由になりうる（手段が明確なだけで、真の課題への最善とは限らない — 下の代案提案の例を参照）
- **そのまま進める**（掘らずに着手）に寄せる目安: 純粋作業（タブ削除・typo 的な文言修正・整形）／ゴールが明確で手段はお任せ（「任せる」「やり方は問わない」）／掘っても手段が変わらなそう、のとき

この見立ては当てにいくものではなく、外すことがある（掘るべきか進めるべきかを機械的に見分けるのは難しい）。だから独断で決めず、**見立てを述べて依頼者に確認する** — この確認が見立てのブレを受け止める安全網。確認は `AskUserQuestion` で「そのまま進める / 深掘る」を選択肢として出し、自分の見立てを推奨に置く。依頼者は自分の意図（探ってほしいのか、任せて進めてほしいのか）を分かっているので選べる。

- **依頼者が「そのまま進める」を選んだ** → 「何をするか」「どうなったら完了か」の最小限だけ口頭で揃え、`basic-design` skill を挟まず `slice-tdd` skill での実装にそのまま渡す（最小合意を口頭で持って渡す）。要件定義.md は作らない。**この時点で終わり** — 下の「やること」以降の深掘りセクションは実行しない
- **依頼者が「深掘る」を選んだ** → 下の通常フローに進む

迷ったら深掘る側を推奨に置く。「簡単そうで難しい／指定された手段が最善でない」を取りこぼさないため。

## やること（＝深掘ると決めた場合の通常フロー）
...
- **自己完結性は handoff-docs を呼んで効かせる**: 要件定義.md は別セッションのエージェントが読んで設計・実装に進む成果物。会話文脈ゼロで読んで自走できる自己完結性 (Why・決定の根拠・前提・制約・スコープ境界・却下案を漏らさず、文脈なしで読める表現) の規律は `handoff-docs` skill が単一の出典として持つ。要件定義.md を書くときはこの skill を呼んで規律を効かせる (自己完結性のルールをここに二重に書かない)
