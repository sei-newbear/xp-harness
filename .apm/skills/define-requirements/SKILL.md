...
- **自己完結性は handoff-docs を呼んで効かせる**: 要件定義.md は別セッションのエージェントが読んで設計・実装に進む成果物。会話文脈ゼロで読んで自走できる自己完結性の規律は `handoff-docs` skill が単一の出典として持つ。要件定義.md を書くときはこの skill を呼んで規律を効かせる (自己完結性のルールをここに二重に書かない)
