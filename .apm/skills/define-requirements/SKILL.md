...
## まず「軽い／重い」を判定して、判断を述べて確認する ★最初に通す

発火したら、いきなり Why 掘りや下の 4 セクションの要件定義に入らない。まず依頼のインテントを軽く読み、2 つの軸で軽いか重いかを判断する:

- **軽量か**: 変更が小さく閉じているか（純粋作業や、1 箇所に閉じた小さな修正など）
- **要件が固まっているか**: 何をどうしたいか・どうなったら完了かが依頼時点で明確で、表面の依頼と真の課題がズレていなさそうか

2 軸はどちらも満たして初めて「軽いかも」。最終的に軽い側に置くのは、下の「判定の芯」の純粋作業に当たるときだけ（迷ったら重い側）。

そして、軽い・重いの**どちらに転んでも、自分の判断を述べて依頼者に確認する**（片方向だけの確認にしない）。確認はクローズな選択なので `AskUserQuestion` を使う:

- **軽いと判断した** → 「これは軽いと判断したので、要件を深掘りせずこのまま進めますね。いいですか?」と確認する
  - 依頼者が了承 → 「何をするか」「どうなったら完了か」の最小限だけ口頭で揃えて実装フェーズに渡す。要件定義.md は作らない。**この時点で終わり** — 下の「やること」以降の重いセクションは実行しない
  - 依頼者が「固めたい」→ 下の通常フローに進む
- **重いと判断した** → 「これは固めた方がよさそうなので、Why から掘って要件を固めますね。いいですか?」と確認する
  - 依頼者が了承 → 下の通常フローに進む
  - 依頼者が「軽くていい」→ 上の軽いパス（最小合意で着手）に倒す

### 「解決策が指定されている」を「要件が固まっている」と取り違えない（判定の芯）

「○○を表示して」「○○を追加して」のように解決策が具体的に指定されていても、それは解決策が明確なだけで、真の課題が固まっている証拠ではない（裏に別の課題が隠れ、別の解決策の方が良いことがある — 下の代案提案の例を参照）。解決策の指定は「要件が固まっている」に数えず、重い側に寄せて判断する。純粋作業（タブ削除・typo 的な文言修正・整形）だけを軽い側に置く。判定に迷ったときも重い側に倒す。「簡単そうで難しい」を取りこぼさないための芯。

## やること（＝しっかり固める場合の通常フロー）
