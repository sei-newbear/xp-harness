...
開けた小さい人形も、同じ「契約を先に決める」動き。**ストーリーの境界だけでなく、実装の中の各層でも** これを効かせる。
...
失敗するテストを 1 個書く。テストの粒度は単位による（E2E spec なら `e2e` skill を呼ぶ／ロジック単位ならユニットテスト／どのレベルかは単位の主題で決める＝project の流儀）。書いたら実行し、**期待した理由で失敗する**ことを確認する（import エラー等の事故的失敗は数えない）。
