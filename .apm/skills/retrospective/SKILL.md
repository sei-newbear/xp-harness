...
深掘りのループを抜けたら (= もう深掘りするトピックがない)、成果物の記録に入る前に、セッション全体をスイープして「取り決めへの書き戻し候補」を出す。依頼者に「スキルに反映することはないか」と聞かれるのを待たず、ふりかえりの一部として必ず通す (= 依頼者の聞き忘れで知識が消えるのを防ぐ)。

- **深掘りのアクションとして既に取り決めへの接続が出ているものは再掲しない**。この段の候補と深掘りのアクションを合わせたものが、このセッションの書き戻し候補の全量になる (= 分断でなく補完)
- 観測が **評価のレンズ** (よかった点 / 伸びしろ) でセッションを見るのに対し、この段は **収穫のレンズ** (次セッションに残すべき知識・手順) で見る。同じスイープの機械的な繰り返しではない
- 拾う対象は 2 種類とも拾う: **依頼者から受けた指摘・修正・好みの伝達の蓄積** / **自分が作業中に見つけた手順・調べ方・ノウハウ** (= うまくいったやり方。どちらも失敗にも伸びしろにも分類されず、深掘りを経ないまま流れやすい知識の受け皿)
- main session が先に候補を出し、依頼者が追加 / 削除 / 修正する (= 観測と同じ共同編集の型)
- 候補の形式 (= どの取り決め (どの skill / subagent / ルール) に、何を書き戻すか)・ここで実装しないこと・0 件は「(なし)」と明示すること、はアクション段と同じ流儀
...
- アクション 0 件は「(なし)」と明示
- 「取り決めへの書き戻し候補」section は深掘り section 群の後に置く。深掘りのアクションに出たものは再掲しない、0 件は「(なし)」と明示
