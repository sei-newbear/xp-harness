...
description: |
  ...
  役割: 現 branch / remote 同期 / working tree の確認、新 worktree / branch 提案、完了時の main 統合、push 規律、`gh` 禁止、consumer 自前 instruction での部分上書き対応。
...
Project 固有の Git 運用ルール (branch protected、commit message 規約、push 禁止 branch、PR 必須運用など) は、consumer 側の自前 instruction (`.apm/instructions/<own>-git-rules.md` 等) に書く。`apm compile` で CLAUDE.md に bundle され、git-workflow skill は CLAUDE.md の該当 section を読んで動作を切り替える。

**書かれた部分だけ上書き、書かれていない部分はこの skill の既定動作に従う** (部分上書きが標準)。

例: consumer の instruction に「PR 必須 (main への直 push 禁止)」だけ書かれていれば、完了時の統合提案は出さず「branch のまま push (PR 運用)」を既定にし、それ以外 (worktree 提案 / commit リズム / 一般規律) はこの skill の既定で動く。
