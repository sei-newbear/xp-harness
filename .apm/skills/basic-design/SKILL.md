...
- **自己完結性は handoff-docs を呼んで効かせる**: 基本設計.md は別セッションのエージェントが読んで実装に進む成果物。会話文脈ゼロで読んで自走できる自己完結性の規律は `handoff-docs` skill が単一の出典として持つ。基本設計.md を書くときはこの skill を呼んで規律を効かせる (自己完結性のルールをここに二重に書かない)
