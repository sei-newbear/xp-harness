...
| # | TODO | 温度感 |
|---|---|---|
| 1 | self-host (dogfooding) の実現 | 開発周り、優先度低 |
| 2 | `.apm/instructions/main.md` の section 分割精査 | 中身を精査しながら判断 |
| 3 | cross-agent 対応 (Cursor 等への移植) | 痛みが見えてから |
| 4 | 初回 install 時の CLAUDE.md 退避カスタムコマンド | 利用者の事故防止 |
| 5 | git-workflow skill の `gh` 禁止 / PR 作成依頼者責務の汎用性検討 | 個人 / チーム portability では現状で OK |
| 6 | done-verifier のテスト/build/source dir 言及の完全除去 | 開発周り |
| 7 | e2e-reviewer の e2e skill 規約言及完全除去 | 開発周り |
| 8 | e2e skill を framework 中立化 (Playwright 前提を外す) | Playwright 以外を使う consumer が増えてから |
| 9 | e2e skill を思想 (e2e-philosophy) と実装 (e2e-playwright) に分離 | skill 増えてきたら検討 |
| 10 | 公式 skill-creator では足りない高度な機能 + skill-template 同梱 | skill 開発が増えてきたら |
| 11 | philosophy skill (改修者向け) を consumer に誤配布しない仕組み | 誤配布シナリオが実際に起きたら |
| 12 | skill 開発の TDD / eval 方法論導入 | チーム配布が見えてきたら |
| 13 | デバッグスキル新設 (systematic-debugging 相当) | バグ修正の質に痛みが見えてから |
| 14 | skill 編集時の思想 loading 仕組み | skill が増えるほど drift リスク累積 |
| 15 | AgentTeam 導入検討 | 現行 subagent の検証が進んでから |
| 16 | code-reviewer の description に設計書/要件定義パスを渡す前提を明示 | レビューでの気づき (優先度未判断) |
| 17 | code-reviewer のレビュー範囲を diff だけに閉じず周辺コードも refactor 対象に | レビューでの気づき (優先度未判断) |
| 18 | e2e-reviewer からプロジェクト固有値 (src/ specs/ beforeEach globalSetup 等) のにじみ出しを除去 | レビューでの気づき (優先度未判断) |
| 19 | e2e-reviewer の「テスト独立性」section が重複しているのでまとめる | レビューでの気づき (優先度未判断) |
| 20 | e2e-reviewer の description に設計書/要件定義パスを渡す前提を明示 | レビューでの気づき (優先度未判断) |
| 21 | pre-implementation-reviewer の description に要件定義/基本設計パスを渡す前提を明示 | レビューでの気づき (優先度未判断) |
| 22 | main.instructions.md のフェーズフロー section の削除またはスリム化 | レビューでの気づき (優先度未判断) |
| 23 | main.instructions.md の各フェーズの振る舞い section の skill 重複解消検討 | レビューでの気づき (優先度未判断) |
| 24 | main.instructions.md 全体の skill 重複洗い出し / 必要なものへの絞り込み (章ごと + 全体) | レビューでの気づき (優先度未判断) |
| 25 | ふりかえり skill の新設 (作業完了後にユーザーへふりかえりを問いかけ、改善点を自発的に促す) | レビューでの気づき (優先度未判断) |
| 26 | basic-design / define-requirements の末尾「/clear で次フェーズへ」案内の要否検討 | レビューでの気づき (優先度未判断) |
| 27 | E2E と対になる実装 skill の新設 (実装規約 + architecture を担う) | レビューでの気づき (優先度未判断) |
| 28 | define-requirements / basic-design と dialogue-principles の重複整理 | レビューでの気づき (優先度未判断) |
| 29 | e2e skill の description から具体名 (Playwright, test.use, getByRole, page.locator) を除去 | レビューでの気づき (優先度未判断) |
| 30 | git-workflow skill の description から `gh` 不使用ルール言及を除去 (具体ツール名 NG) | レビューでの気づき (優先度未判断) |
| 31 | git-workflow skill 本文から `.apm` / `apm compile` / consumer 等の APM 機構言及を除去 | **優先度高** |
| 32 | slice-tdd skill の名前再検討 | **優先度高め** |
| 33 | slice-tdd skill 内の Playwright 記述 (L215) を除去 | レビューでの気づき (優先度未判断) |
| 34 | slice-tdd skill の「依頼者に状況を確認する」箇所を自走モード前提で詰める | レビューでの気づき (優先度未判断) |
| 35 | slice-tdd skill に「実装関連 skill があれば参照する」指示を追加 (TODO 27 の繋ぎ) | **優先度高め** |
| 36 | code-reviewer subagent に skill 動的発見・適用機構を追加 (preload 以外で skill を見つけられるなら) | レビューでの気づき (優先度未判断) |
| 37 | story-slicing の呼び出し指示を define-requirements 側に移す (発火不全の仮説対応) | レビューでの気づき (優先度未判断) |
...
## TODO 7: e2e-reviewer の e2e skill 規約言及完全除去

### 状況

e2e-reviewer subagent は preload した e2e skill から規約を借りる設計だが、本文には e2e skill 内の例 (「natural language ヘルパー」「注文履歴」等) への言及が残っている。理想は subagent 本文を完全に観点だけにして、規約は全て preload された skill 側から借りる純化。

### 再開時の起点

1. e2e-reviewer 本文を読み返し、e2e skill の中身を引いている箇所を識別
2. preload skill から動的に参照する形に書き換え (「e2e skill の Rule N を参照」)
...
## TODO 18: e2e-reviewer からプロジェクト固有値 (src/ specs/ beforeEach globalSetup 等) のにじみ出しを除去

### 状況

`.apm/agents/e2e-reviewer.md` 本文に、プロジェクト固有 / framework 固有の語が直接書かれている:

- `specs/` (L19) — ディレクトリ命名規約はプロジェクト側の話
- `src/` (L68) — 同上
- `beforeEach` (L48, L62) — Playwright / Jest 系 framework 固有 API 名
- `globalSetup` (L64) — 同上
- `afterEach` (L62) — 同上

reviewer 自身は規約を持たず preload した `e2e` skill から借りる、と L23 で明言しているのに、本文側にこれらが直に書かれているのは設計意図と矛盾する。TODO 7 (「e2e-reviewer の e2e skill 規約言及完全除去」) と同根の話だが、ディレクトリ名 / framework API 名のにじみ出しという観点で具体的に切り出す。

### 再開時の起点

1. e2e-reviewer 本文から `src/` / `specs/` / `beforeEach` / `afterEach` / `globalSetup` 等の固有語を識別
2. preload した `e2e` skill 側の規約名 (例: 「e2e skill の『テスト独立性』規約を参照」) で参照する形に書き換え
3. TODO 7 とのマージ可否を検討 (内容が近いため、起票時にまとめて対応するのが効率的かもしれない)
...
## TODO 36: code-reviewer subagent に skill 動的発見・適用機構を追加 (preload 以外で skill を見つけられるなら)

### 状況

現状 code-reviewer subagent は `skills:` frontmatter で `slice-tdd` のみ preload。**subagent が preload 以外にも実行時に関連 skill を発見・適用できるなら**、code-reviewer に「レビュー対象のコードと文脈から関連 skill (実装規約 skill / framework 固有 skill / その他) を探して読み、それに基づいてレビューする」指示を入れたい。

前提: subagent が preload 以外の skill を自律的に load できる機構があるか先に確認する必要がある (Claude Code の subagent 仕様)。出来ない場合は preload リストを拡張する代替案で対処。

TODO 27 (実装 skill 新設) / TODO 35 (slice-tdd 経由の暫定参照) と連動する話。code-reviewer 側も実装 skill を参照できると、レビュー基準が「単一の SOLID 観点」だけでなく「project 固有規約」「実装 skill 規約」とも整合的に走るようになる。

### 再開時の起点

1. subagent が preload 以外の skill を runtime で load 可能かを Claude Code 仕様で確認 (現状 `skills:` frontmatter 以外で skill を発火させる手段があるか)
2. 可能なら code-reviewer 本文に「関連 skill を探して参照する」指示を追加
3. 不可能なら preload リストを拡張する代替案 (`skills: [slice-tdd, implementation, ...]`) を検討
4. TODO 27 完成後に正式な preload に組み込む流れも視野に入れる
