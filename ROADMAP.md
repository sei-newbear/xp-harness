...
**TODO 31 (git-workflow 本文 APM 言及除去) は完了 (2026-06-13)**: APM 言及除去に加え、全 5 skill の `CLAUDE.md` ハードコードを agent 非依存化。上書き効果を維持するため効き目アンカー箇所は「project の instruction (Claude Code なら CLAUDE.md)」という折衷表現で対応。sandbox 検証（条件A / 条件B 比較）でアンカーあり→commit 自走 / なし→commit なしを確認。
