...
- **完了時の統合方法は利用者影響で切り分ける** (= git-workflow の「main へ統合が既定」を上書き)。**利用者影響がある変更** (= `.apm/` 配下の配布物に触れるもの) は main へ直接統合せず、branch を push して **PR を出しレビューを通す**。**利用者影響がない変更** (= 改修者向け `.claude/` 配下・`docs/`・`ROADMAP.md`・`CLAUDE.md` 等のみ) は main へ直接統合してよい
