...
「自動 worktree」はかつてこの象限 (痛みが明確化していない / Git 運用が固まってから) に置かれていたが、並行作業の痛み (Claude の自走中に利用者が同じ作業ツリーで別作業できない) が明確化したため、git-workflow の worktree 化として採用した (経緯は `docs/working/git-workflow-worktree/要件定義.md`)。
