...
description: xp-harness の skill / agent (`.apm/skills/<name>/SKILL.md` / `.apm/agents/<name>.md`) や改修者向け skill / agent (`.claude/skills/` / `.claude/agents/` 配下に直接コミットされたもの) を新規作成・改修するときに必ず発火させる改修者向け skill。skill 設計の流儀 (= 構造の理解 / 境界原則 / description の書き方 / 出力前に立ち止まる) と判断軸 (= 投資フェーズ / E2E 未確立 / subagent 3 用途 / スコープ境界 .apm vs .claude / 機械的処理に流れる失敗の認識) と改修フロー (= 改修後に skill-reviewer に通す) を集約する。CLAUDE.md (project root) から埋め込み参照される改修者向けの規律 source として機能する。consumer 配布対象外 (= `.claude/skills/` 配下に直接 git tracked、philosophy skill と同じパターン)。
---
...
- main session (= 改修者の役を担うコーディング Agent、現時点では Claude Code 等)
- skill-reviewer (= preload skill としてこの内容を context に注入される)

この skill 自体は consumer に配布されない (= `.claude/skills/skill-design-style/` 配下に直接 git tracked、philosophy skill と同じパターン)。
...
skill / agent を作る・改修するときの 3 つの境界原則。
...
#### 機械的処理に流れる失敗の認識 (= 自己観察項目)

「これはルーチン作業」「決まったパターンの繰り返し」と分類した瞬間に立ち止まる。以下の自己観察項目を出力前に通す:

- **自分の発想軸に縛られていないか?** — 別角度から発想できるか、1 つの案をずらすだけになっていないか
- **確認を skip していないか?** — 依頼者に確認する前に自分で読んで確認しているか、既存の resolved 状況を Read で確認したか
- **省略していないか?** — 要件定義書のナンバリング / 案番号 / 略号で語らない、対話では内容で語る
...
これらの項目は dialogue-principles skill にも書かれている (= 対話の場面では dialogue-principles が core)。skill 改修と対話は同時に起きるので、両方の skill を併用する。
...
- **skill-reviewer**: skill SKILL.md (description / 本文) のレビュー専門
- **code-reviewer**: 実装コード (= ts / py / js 等のロジック持ちファイル) のレビュー、SKILL.md は対象外
- **e2e-reviewer**: E2E spec のレビュー
- **pre-implementation-reviewer**: 要件・基本設計のレビュー
- **done-verifier**: 完了判定の検証

skill 改修時に code-reviewer も呼ばれた場合 (= git diff に SKILL.md が含まれるため) は、main 裁量でスルーしてよい (= ダブった指摘の観点ズレを main session が判断)。痛みが見えてから構造的対処に移行する経路を残す。
