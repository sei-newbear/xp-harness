...
#### 個別案件で言語化した境界原則は philosophy に昇格させる

これは 4 軸の 3 番目 (= 既出か / 源泉に集約するか) を、個別案件をまたぐ時間軸に広げた運用。個別案件 (`docs/working/<title>/` の要件定義・基本設計) の中で、xp-harness の責務境界・採否に関わる新しい原則を言語化したら、その案件の文書に埋もれさせず、思想・判断軸の源泉である `philosophy` skill の該当 section に昇格させる。

理由: philosophy は CLAUDE.md 経由で常時 context に乗る源泉。個別案件の文書は次の判断のとき読まれない。源泉に昇格させないと、同じ原則を毎回その場で組み立て直し、判断の向きを外す (= 実際に「規約探索型スキルへの薄型化」案件で、決定打の境界原則が要件定義.md にしか無く philosophy に無かったため、main session が責務境界を超える判断を繰り返した)。
...
#### subagent は「本文の読み手」が skill と違う

上の 2 層構造・動作フローは skill の話。subagent (`.apm/agents/*` / `.claude/agents/*`) は **本文を読む主体が別** で、ここが skill と決定的に違う:

- **skill**: description も本文も **同じ main session** が読む (判断前に description、発火後に本文)。読み手は一貫している
- **subagent**: **呼ぶ側 (main session) は description しか読まない**。本文は subagent 起動時にその **subagent 自身の system prompt** になり、subagent だけが読む。呼ぶ側と本文の読み手が別の agent になる

ここから 3 つ導かれる:

- **呼ぶ側に届けたい情報は description に置く**: いつ委譲するか (発火条件) に加え、**どう呼ぶか・何を渡す / 渡さないか** の呼び出し規約も、呼ぶ側に効かせたいなら description に書く (本文に書いても呼ぶ側は読まない)。呼ぶ側を駆動する別 skill があるなら、その skill 本文に置いてもよい。なお原則 1 が description から排除するのは preload / 配布などのメタ情報で、呼ぶ側の委譲判断・呼び方に要る呼び出し規約は interface の一部だから description に置いてよい (両者は別レイヤー)
- **本文は subagent への指示として書く**: 本文の読み手は subagent 自身なので、subagent に向けた指示 (二人称) として書く。main session 向けの発火条件を本文に書かない (= 原則 3 の subagent 版)
- **共有したい規律は preload で効かせる**: 複数の subagent / main session と共有したい哲学・規律は、本文に再掲せず preload skill で subagent の context に注入する (= 規律装置最小注入、single source を保つ)。どの subagent が何を preload するかは agent 定義側で指定する
...
- 最大 1,024 字 (公式の上限)。**200-300 字は目安として xp-harness が置く独自基準** (= 公式は「Concise is key、description は skill 選択に効く」と言うが、200-300 という数字自体は公式ではない)
...
上の字数の目安 (200-300 字) は skill の話。**subagent の description には公式の長さ推奨が無い** (公式は「Claude が委譲を判断するのに使うので、いつ使うか分かる明確な description を書け」だけ)。だから subagent は字数で機械的に削らず、「呼ぶ側が **委譲判断** と **正しい呼び方** に必要な情報が揃っているか」で判断する。呼び出し規約 (何を渡す / 渡さないか) を字数のために削って呼ぶ側に届かなくするのは本末転倒 (= 手順 2 の「subagent は本文の読み手が違う」の帰結)。
...
**必須の 1 回に加えて追加で呼ぶかどうかは main session の判断で、依頼者の許可を取る事項ではない**: 変更が一段落するたび・迷うたびに気軽に呼ぶ。「skill-reviewer に通しますか？」と依頼者に委ねない (= 気軽に呼ぶは判断委任であって許可取りではない)。subagent 起動のコストや「今日もう 1 回やった」を理由に呼ぶ回数を出し惜しみしない ── code-reviewer を Refactor 前に何度でも反射的に呼ぶのと同じ気軽さで呼ぶ (捕捉の価値 > 呼ぶコスト)。
...
### 手順 7: 挙動に効く改修なら harness-verification で確かめる

改修しても、狙った振る舞いが出るかは **transcript を観測しないと分からない**。発火や振る舞いに効く改修をしたら、`harness-verification` skill を呼んで確かめる。

判断軸は「この改修で main session の振る舞いが変わることを期待しているか」。期待しているなら、確かめずに閉じない (= 期待は仮説で、まだ観測していない)。

要らないのは、typo の修正・見出しの整形・参照先パスの追随など、**読み手の判断材料が変わらない改修**。**「言い回しを整理しただけ」は免除の理由にならない** — description の文言変更は整理のつもりでも発火確率に効く (実測: 旧 3/3・新 0/3 の回帰)。文言を触ったかどうかでなく、読み手の判断材料が動いたかで切る。

**呼ばないと判断したときは、その理由を依頼者に出す** (= 黙って省かない)。重さを理由にするなら、確かめずに閉じることを依頼者が承知した状態にする。

skill-reviewer (手順 6) が見るのは設計の規律で、実際に発火・振る舞いが出るかは見ない。両方を通して初めて改修が閉じる。

### 手順 8: 対話する場面では dialogue-principles を読む
