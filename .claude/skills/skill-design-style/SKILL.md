---
description: xp-harness の skill / agent を新規作成・改修するときに必ず発火させる改修者向け skill。skill 設計の流儀 (構造 / 境界原則 / description の書き方 / 出力前に立ち止まる) と判断軸と改修フロー (改修後に skill-reviewer に通す) を集約する。CLAUDE.md から埋め込み参照され、skill-reviewer に preload される改修者向けの規律 source。consumer 配布対象外。
---
