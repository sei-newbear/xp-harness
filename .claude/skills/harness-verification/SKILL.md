...
- **規律通り走れたか**: 対象フェーズを統べる skill の理想に照らす（実装なら slice-tdd の red→green、reviewer / done-verifier を仕上げ前に通したか、commit 粒度、確認なしに push していないか 等）
...
- **成果密度**: その稼働時間で何が完成したか（commit）と時間の内訳（実装 / レビュー待ち / 検証待ち）。**生産性そのものの計測ではなく**、空転（実装が進まず待ちに時間が溶ける）を skill 連携・リズムの不全のサインとして観る（harness の効きの裏返し）
...
### <シナリオ A>
- 何を検証したか: 対象 skill / 条件 (プロンプト) / 反証条件
- 結果: 観測された振る舞い
