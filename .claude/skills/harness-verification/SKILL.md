...
- **人間の介入点は 2 種類ある**: 自由記述の user メッセージだけでなく、`AskUserQuestion` の回答は user 側の `tool_result`（"have been answered" を含む）として乗る。片方だけ数えると介入回数を取りこぼす（実際、選択肢回答を除外して数え、自走時間を過大評価した失敗がある）。remote-control 経由でも記録形は同じ（remote-control は入力を隠さない）
