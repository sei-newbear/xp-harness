...
「内容の書きっぷり」(= 観測可能な挙動の変化で書く) と対になる量の規律。**長いリリースノートは読まれず、読まれなければ書いた意味がない**。

- **ユーザー向けの 1 項目は 1〜2 文**。太字の見出し的な 1 文で「何が変わったか」を言い切り、必要なら補足を 1 文足す。**収まらないときは項目を分けるのでなく、内部の how を落として観測される変化だけ残す**
- **長くなる主因は skill の内部構成の説明**。「これまでは A だけだったところに B が加わった」と書き始めたら、書きっぷりの規律を通し損ねているサイン
- **項目を機械的に増やさない**。1 つの変更を「skill への追加」「instruction への追加」と書く場所ごとに分けると、使う人には同じ 1 つの変化にしか見えない。使う人が観測する単位でまとめる
- 開発者向けは 1 項目 1 文でよい (= 改修者は差分を直接読める)

判断軸: 「ユーザー向け section を、consumer が上から下まで一息で読み切れるか?」項目ごとに読み返しが要る長さなら削る。あわせて「今回の変更の重さに対して、前回のリリースノートより厚くなっていないか」も見る。
