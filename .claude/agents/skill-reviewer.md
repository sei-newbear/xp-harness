...
### 5. スコープ境界の判別 (= 利用者向け vs 改修者向け、skill-design-style の判別法に従う)

skill-design-style の「スコープ境界の判別」section に集約された判別法に従う。重要なのは:

- **配置パスから推測しない** (= `.claude/` 配下でも symlink なら `.apm/` と同じ実体で利用者向け、dogfooding のため symlink が貼られている)
- **必ず `ls -la <対象ファイル>` で symlink かどうかを Bash で確認する** (= 推測ではなく実行ベースで判別)
- symlink → 利用者向け、symlink でない (= 直接 git tracked) → 改修者向け
